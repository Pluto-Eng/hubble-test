...
import { LoanApplicationStatus } from '@/lib/types/loan';
...
export function getStepFromStatus(status: LoanApplicationStatus): number {
  switch (status) {
    ...
    case 'approved':
      return 5; // Contract
    case 'rejected':
      return 3; // Back to submit for corrections
    case 'completed':
      return 5; // Contract
    case 'cancelled':
 ...
 */
export function isValidLoanApplicationStatus(status: string): status is LoanApplicationStatus {
  ...
  return validStatuses.includes(status as LoanApplicationStatus);
}
